...
class _BracketScreenState extends ConsumerState<BracketScreen> {
  ...
  void _updateFitScale(
    MacmahonState state,
    BoxConstraints constraints,
    List<PairingResult?> displayRounds,
    int totalRounds,
    int qCount,
  ) {
    ...
    if (currentData.bracketStyle == BracketStyle.compact) {
      ...
      final isVertical =
          currentData.bracketStyle == BracketStyle.classicVertical;
      if (isVertical) {
  ...
  Widget build(BuildContext context) {
    ...
    bool isKnockoutMatch(PairingResult r) {
      ...
      final allKnockoutPlayers = r.pairs.every(
        (p) =>
    ...
    final currentIsKnockout =
        (currentData.stage == 2 || state.format == TournamentFormat.knockout) &&
        currentPairing != null;
    ...
    final totalRounds = qCount > 1
        ? (math.log(qCount) / math.log(2)).ceil()
        : 1;
      ...
      if (_isManualMode) {
        final allPossible = qualifiers.isNotEmpty
            ? state.currentSectionPlayers
                  .where((p) => qualifiers.contains(p.id))
                  .toList()
            : state.currentSectionPlayers;

        _draftPlayers ??= List.filled(qCount, null);
        final usedIds = _draftPlayers!
            .where((p) => p != null)
            .map((p) => p!.id)
            .toSet();
        final available = allPossible
            ...
            Row(
              children: [
                ...
                SegmentedButton<DraftStyle>(
                  segments: const [
                    ButtonSegment(
                      value: DraftStyle.table,
                      label: Text('테이블형'),
                      icon: Icon(Icons.table_rows_rounded),
                    ),
                    ButtonSegment(
                      value: DraftStyle.direct,
                      label: Text('직관형(드래그)'),
                  ...
                  onSelectionChanged: (set) =>
                      setState(() => _draftStyle = set.first),
                ),
                const SizedBox(width: 16),
                ElevatedButton(
                  onPressed: _draftPlayers!.any((p) => p != null)
                      ? () async {
                          final finalOrder = _draftPlayers!
                              .map<MacmahonPlayer>(
                                (p) =>
                                    p ??
                                    MacmahonPlayer(
                          ...
                          await ref
                              .read(macmahonProvider.notifier)
                              .generateManualPairing(finalOrder);
                          if (mounted) {
                            setState(() {
                              _isManualMode = false;
                              _draftPlayers = null;
                              ...
                              itemBuilder: (context, index) {
                                ...
                                return DragTarget<MacmahonPlayer>(
                                  onAccept: (data) => setState(() {
                                    for (
                                      int i = 0;
                                      i < _draftPlayers!.length;
                                      i++
                                    ) {
                                      if (_draftPlayers![i]?.id == data.id)
                                        _draftPlayers![i] = null;
                                    }
                                  ...
                                  builder: (context, candidate, _) => ListTile(
                                    ...
                                    trailing: selected != null
                                        ? IconButton(
                                            ...
                                            onPressed: () => setState(
                                              () =>
                                                  _draftPlayers![index] = null,
                                            ),
                                          )
                                        : null,
                                    tileColor: candidate.isNotEmpty
                      ...
                      child: Center(
                        child: FittedBox(
                          ...
                          child: _DraftPreviewBracket(
                            ...
                            style:
                                currentData.bracketStyle == BracketStyle.compact
                            ...
                            onPlayerDropped: (index, player) => setState(() {
                              for (int i = 0; i < _draftPlayers!.length; i++) {
                                if (_draftPlayers![i]?.id == player.id)
                                  _draftPlayers![i] = null;
                              }
                              _draftPlayers![index] = player;
                            }),
                            onPlayerRemoved: (index) =>
                                setState(() => _draftPlayers![index] = null),
                          ),
                        ),
                      ),
                    ),
        ...
        child: ConstrainedBox(
          ...
          child: Column(
            ...
            children: [
              ...
              const Text(
                '본선 진출자 명단을 확인하고 대진 생성 방식을 선택하세요.',
            ...
            children: [
              SegmentedButton<BracketStyle>(
                segments: [
                  if (!_isManualMode)
                    const ButtonSegment(
                      value: BracketStyle.compact,
                      label: Text('컴팩트', style: TextStyle(fontSize: 12)),
                      icon: Icon(Icons.grid_view_rounded, size: 16),
                    ),
                  const ButtonSegment(
                    value: BracketStyle.classic,
                    label: Text('클래식(가로)', style: TextStyle(fontSize: 12)),
                    icon: Icon(Icons.account_tree_rounded, size: 16),
                  ),
                  const ButtonSegment(
                    value: BracketStyle.classicVertical,
                    label: Text('클래식(세로)', style: TextStyle(fontSize: 12)),
                ...
                onSelectionChanged: (Set<BracketStyle> newSelection) {
                  ref
                      .read(macmahonProvider.notifier)
                      .updateBracketStyle(newSelection.first);
                },
                    ...
                    : Listener(
                        onPointerSignal: (pointerSignal) {
                          if (pointerSignal is PointerScrollEvent) {
                            ...
                            final Matrix4 matrix = _transformationController
                                .value
                                .clone();
                            final double currentScale = matrix
                                .getMaxScaleOnAxis();
                            final double targetScale =
                                (currentScale * scaleDelta).clamp(0.001, 5.0);
                ...
                child: ElevatedButton.icon(
                  onPressed: currentRoundDone
                      ? () async {
                          ...
                          final newHist = s.history
                              .where(isKnockoutMatch)
                              .toList();
            ...
            onPressed: () async {
                ...
                child: Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      Text(
                        '${state.selectedSection} - 토너먼트 대진표',
              ...
              if (path != null && mounted) {
                ScaffoldMessenger.of(
                  context,
                ).showSnackBar(SnackBar(content: Text('이미지가 저장되었습니다: $path')));
              }
            },
          ),
          IconButton(
...
class _CompactBracketTree extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    for (int r = 0; r < n; r++) {
      ...
      for (int m = 0; m < round.pairs.length; m++) {
        ...
        if (p.isResultEntered && p.winnerId != null)
          winnerMap[r]![m] = p.winnerId == p.black.id
              ? p.black.name
              : p.white.name;
      }
    }
    ...
    for (int r = 0; r < n; r++) {
      final round = displayRounds[r],
          y = yTop(r),
          expected = math.pow(2, n - 1 - r).toInt(),
          isCurrent = r == currentRoundIdx;
      ...
      for (int m = 0; m < expected; m++) {
        final xc = cx(r, m),
            pair = (round != null && m < round.pairs.length)
                ? round.pairs[m]
                : null;
        String? pA = pair != null
            ? pair.black.name
            : (r > 0 && winnerMap.containsKey(r - 1)
                  ? winnerMap[r - 1]![m * 2]
                  : null);
        String? pB = pair != null
            ? pair.white.name
        ...
        if (pair != null && pair.isResultEntered && pair.winnerId != null)
          winner = pair.winnerId == pair.black.id
              ? pair.black.name
              : pair.white.name;
...
class _ClassicBracketTree extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    for (int m = 0; m < expectedLeafs / 2; m++) {
      final pair = (round1 != null && m < round1.pairs.length)
          ? round1.pairs[m]
          : null;
      nodes[0].add(
        _NodeData(
          name: pair?.black.name,
          isWinner: pair?.winnerId == pair?.black.id && pair?.winnerId != null,
          pair: pair,
          isCurrentMatch: currentRoundIdx == 0,
        ),
      );
      nodes[0].add(
        _NodeData(
          name: pair?.white.name,
          isWinner: pair?.winnerId == pair?.white.id && pair?.winnerId != null,
          pair: pair,
          isCurrentMatch: currentRoundIdx == 0,
        ),
      );
    ...
    for (int level = 1; level < totalLevels; level++) {
      final round = level <= displayRounds.length
          ? displayRounds[level - 1]
          : null;
      final nextRound = level < displayRounds.length
          ? displayRounds[level]
          : null;
      final expectedNodes = math.pow(2, n - level).toInt();
      for (int m = 0; m < expectedNodes; m++) {
        final pair = (round != null && m < round.pairs.length)
            ? round.pairs[m]
            : null;
        String? winnerName;
        if (pair != null && pair.isResultEntered && pair.winnerId != null)
          winnerName = (pair.winnerId == pair.black.id)
              ? pair.black.name
              : pair.white.name;
        bool isWinner = false;
        if (nextRound != null) {
          ...
          if (nextMatchIdx < nextRound.pairs.length) {
            ...
            if (nextMatch.isResultEntered &&
                (nextMatch.winnerId == pair?.winnerId &&
                    pair?.winnerId != null))
              isWinner = true;
          }
        } else if (level == n)
          isWinner = pair?.winnerId != null && pair?.isResultEntered == true;
        bool isCurrent = (level - 1) == currentRoundIdx;
        MacmahonPair? activePair = pair;
        if (!isCurrent &&
            level == currentRoundIdx &&
            nextRound != null &&
            m < nextRound.pairs.length * 2) {
          isCurrent = true;
          activePair = nextRound.pairs[m ~/ 2];
        }
        nodes[level].add(
          _NodeData(
            name: winnerName,
    ...
    double cy(int level, int m) {
      if (isVertical)
        return (totalLevels - 1 - level) * (kH + kVGap) + 200; // 여백 확대
      final step = math.pow(2, level).toDouble();
    ...
    for (int level = 1; level <= n; level++) {
      ...
      if (isVertical) {
        ...
        if (xPositions.isNotEmpty &&
            xPositions.last != cx(level, expectedNodes - 1)) {
      ...
      for (final xPos in xPositions) {
        final double labelY = isVertical ? cy(level, 0) + kH + 25 : 40.0;
    ...
    for (int level = 0; level < totalLevels; level++) {
      for (int m = 0; m < nodes[level].length; m++) {
        ...
        cards.add(
          Positioned(
            ...
            child: _ClassicNodeBox(
              ...
              onTap: (node?.pair != null && node!.isCurrentMatch)
                  ? () => onMatchTap(node.pair!)
                  : null,
            ),
          ),
        );
        ...
        if (level < n) {
          if (isVertical) {
            final nextX = cx(level + 1, m ~/ 2),
                nextY = cy(level + 1, m ~/ 2),
                midY = y - kVGap / 2;
            ...
            if (m % 2 == 0) {
              ...
              lines.add(
                _Line(midX, nextY + kH / 2, nextX - kW / 2, nextY + kH / 2),
              );
            }
          }
        }
    ...
    final totalWidth = isVertical
        ? (kW + kHGap) * expectedLeafs + 400
        : cx(n, 0) + kW + 400;
    final totalHeight = isVertical
        ? (totalLevels) * (kH + kVGap) + 400
        : cy(0, expectedLeafs - 1) + kH + 400;
...
class _ClassicNodeBox extends StatelessWidget {
  final String? name;
  final bool isWinner, isLeaf, isChampion, isCurrent;
  final VoidCallback? onTap;
  const _ClassicNodeBox({
    this.name,
  ...
  Widget build(BuildContext context) {
    ...
    final Color borderColor = isCurrent
        ...
        : (isWinnerActive
              ...
              : (isLeaf ? Colors.blue.shade300 : Colors.grey.shade300));
          ...
          child: Column(
            children: [
              _SlotRow(
                ...
                isLoser:
                    isCompleted && winnerName != playerA && playerA != null,
                label: '흑',
              ),
              ...
              _SlotRow(
                ...
                isLoser:
                    isCompleted && winnerName != playerB && playerB != null,
                label: '백',
              ),
            ],
          ),
...
class _SlotRow extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final Color textColor = isEmpty
        ? Colors.grey.shade300
        : (isLoser ? Colors.grey.shade400 : Colors.black);
    return Expanded(
      child: Container(
            ...
            Container(
              ...
              child: Center(
                child: Text(
                  label[0],
                  style: TextStyle(
                    color: label == '흑' ? Colors.white : Colors.black87,
                    fontSize: 10,
                    fontWeight: FontWeight.bold,
                  ),
                ),
...
class _DraftPreviewBracket extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final n = totalRounds,
        ...
        cards = <Widget>[],
        lines = <_Line>[],
        isClassic = style == BracketStyle.classic,
        isClassicVertical = style == BracketStyle.classicVertical,
        expectedLeafs = math.pow(2, n).toInt();
    final bool isCompact = !isClassic && !isClassicVertical;

    double cx(int level, int m) {
      if (isClassic) return level * (kW + 80.0) + 100;
      final step = math.pow(2, level).toDouble();
      if (isClassicVertical)
        return (m * step + (step - 1) / 2.0) * (kW + 40.0) + 100;
    ...
    double cy(int level, int m) {
      ...
      if (isClassicVertical)
        return (totalLevels - 1 - level) * (kH + 60.0) + 120;
    ...
    for (int level = 0; level < totalLevels; level++) {
      ...
      if (isCompact) {
        if (level == 0)
          expectedNodes = math.pow(2, n - 1).toInt();
        else
          expectedNodes = math.pow(2, math.max(0, n - 1 - level)).toInt();
      ...
      for (int m = 0; m < expectedNodes; m++) {
        ...
        if (isLeaf && isCompact) {
          ...
          final nameA = idxA < draftPlayers.length
              ? draftPlayers[idxA]?.name
              : null;
          final nameB = idxB < draftPlayers.length
              ? draftPlayers[idxB]?.name
              : null;
          ...
          final name =
              (isLeaf &&
                  (isClassic || isClassicVertical) &&
                  m < draftPlayers.length)
              ? draftPlayers[m]?.name
              : null;
          cards.add(
            Positioned(
    ...
    final totalW = isClassic
        ? cx(n, 0) + kW + 200
        : cx(0, expectedLeafs - 1) + kW + 200;
    final totalH =
        (isClassic ? cy(0, expectedLeafs - 1) : cy(0, 0)) + kH * 2 + 200;
    return SizedBox(
      width: totalW,
      height: totalH,
      child: Stack(
...
class _DraftSlot extends StatelessWidget {
  ...
  final String? name;
  final String? label;
  final void Function(int, MacmahonPlayer)? onDropped;
  ...
  Widget build(BuildContext context) {
    return DragTarget<MacmahonPlayer>(
      onAccept: (data) => onDropped?.call(index, data),
          ...
          child: Container(
            decoration: useBox
                ...
                : BoxDecoration(
                    color: isHovering
                        ? AppTheme.primary.withOpacity(0.1)
                        : (name == null ? Colors.grey.shade50 : null),
                  ),
            child: Row(
              mainAxisAlignment: MainAxisAlignment.center,
              children: [
